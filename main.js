...
const buttonOpenOther = document.querySelector("#openOther")
const textLuckCookie = document.querySelector(".text-luck")
...
const handlerToggleEventShake = (e) => {
    e.target.classList.toggle("shake")
}

const handlerTogglePage = () => {
    pageHome.classList.toggle("hidden")
    pageLuck.classList.toggle("hidden")
}

const handlerFortuneCookie = (e) => {
    handlerTogglePage()

    selectedPhrase = 71

    while (selectedPhrase > 70) {
        selectedPhrase = Math.round(Math.random() * 100)
        console.log(selectedPhrase)
    }
    
    console.log("Índice da frase escolhida: " + selectedPhrase)
    textLuckCookie.innerText = phrases[selectedPhrase]

    textLuckCookie.classList.toggle("reveal")
}


fortuneCookie.addEventListener("mouseover", handlerToggleEventShake)
fortuneCookie.addEventListener("mouseout", handlerToggleEventShake)
fortuneCookie.addEventListener("click", handlerFortuneCookie)
buttonOpenOther.addEventListener("click", handlerFortuneCookie)
